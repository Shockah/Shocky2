...
public class CommandQuoteDelete extends CommandWithAliases {
	public CommandQuoteDelete(Module module) {
		super(module);
	}
	
	public String command() {return "quotedelete";}
	public String help() {
		return ".quotedelete [channel] [tags] {id} - deletes a quote";
	}
	...
	public void call(PircBotX bot, ETarget target, CommandCallback callback, Channel channel, User sender, String message) {
		...
		if (aChannel != null) {
			...
			if (aIntId != 0) {
				...
				if (aIntId > count) {
					callback.append("Nothing to delete.");
					return;
				}
				find.skip(aIntId-1);
				...
				module.getCollection().remove(Data.document("_id",doc.get("_id")));
				callback.append("Deleted quote: ["+aChannel+": "+(oaIntId < 0 ? count+oaIntId+1 : oaIntId)+"/"+count+"] "+BotUtils.mungeAllNicks(channel,(String)doc.get("quote")));
				return;
			}
		}
